...
// unclocked ltl.delay operations.
//
...
/// Check if a subtree (rooted at value) contains any unclocked delays.
static bool hasUnclockedDelays(Value value, DenseSet<Operation *> &visited) {
  ...
  if (auto delayOp = dyn_cast<ClockedDelayOp>(defOp))
    return hasUnclockedDelays(delayOp.getInput(), visited);

  if (isa<ClockOp>(defOp))
    return false;

  for (Value operand : defOp->getOperands()) {
    if (hasUnclockedDelays(operand, visited))
      return true;
  }
  return false;
}

/// Recursively clone/update a value's subtree, replacing unclocked delays
...
static Value cloneWithClock(Value value, Value clock, ClockEdge edge,
  ...
  if (auto delayOp = dyn_cast<DelayOp>(defOp)) {
    ...
    // Replace unclocked delay with explicit clocked_delay.
    builder.setInsertionPointAfter(defOp);
    auto newDelay = ClockedDelayOp::create(
        builder, delayOp.getLoc(), newInput, delayOp.getDelayAttr(),
        delayOp.getLengthAttr(), clock,
        ClockEdgeAttr::get(builder.getContext(), edge));
    mapping.map(value, newDelay.getResult());
    return newDelay.getResult();
  }

  // Handle ClockedDelayOp specially.
  if (auto delayOp = dyn_cast<ClockedDelayOp>(defOp)) {
    Value newInput = cloneWithClock(delayOp.getInput(), clock, edge, builder,
                                    mapping, visited);
    if (newInput != delayOp.getInput()) {
      builder.setInsertionPointAfter(defOp);
      auto newDelay = ClockedDelayOp::create(
          builder, delayOp.getLoc(), newInput, delayOp.getDelayAttr(),
          delayOp.getLengthAttr(), delayOp.getClock(), delayOp.getEdgeAttr());
      mapping.map(value, newDelay.getResult());
      return newDelay.getResult();
    }
    mapping.map(value, value);
...
void InferLTLClocksPass::runOnOperation() {
  ...
  for (ClockOp clockOp : clockOps) {
    // Check if this clock op's input subtree has any unclocked delays
    DenseSet<Operation *> checkVisited;
    if (!hasUnclockedDelays(clockOp.getInput(), checkVisited))
      continue;
  ...
  if (changed) {
    ...
    module.walk([&](Operation *op) {
      if (isa<DelayOp, ConcatOp, AndOp, OrOp, IntersectOp, RepeatOp,
              ClockedDelayOp, GoToRepeatOp, NonConsecutiveRepeatOp, NotOp,
              ImplicationOp, UntilOp, EventuallyOp>(op))
  ...
  // Warn about any remaining unclocked delays that could not be inferred.
  module.walk([&](DelayOp delayOp) {
    delayOp.emitWarning("delay has no clock and was not resolved by "
                        "InferLTLClocks; ensure an enclosing ltl.clock "
