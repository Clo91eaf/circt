...
struct DefDrive {
  static DefDrive never() { return Pair(Value{}, DriveNever); }
  static DefDrive always() { return Pair(Value{}, DriveAlways); }
  static DefDrive conditional(Value condition = {}) {
    return Pair(condition, DriveConditional);
  }

  bool isNever() const { return conditionAndMode.getInt() == DriveNever; }
  bool isAlways() const { return conditionAndMode.getInt() == DriveAlways; }
  bool isConditional() const {
    return conditionAndMode.getInt() == DriveConditional;
  }
  ...
  bool operator==(const DefDrive &other) const {
...
private:
  ...
  typedef PointerIntPair<Value, 2> Pair;
  Pair conditionAndMode;

  DefDrive(Pair conditionAndMode) : conditionAndMode(conditionAndMode) {}
  friend DenseMapInfo<DefDrive>;
};
...
struct Def {
  ...
  bool driveConditionIsPlaceholder = false;

  Def(Value value, DefDrive mode)
      : block(value.getParentBlock()), type(value.getType()), value(value),
        mode(mode) {}
  Def(Block *block, Type type, DefDrive mode)
      : block(block), type(type), mode(mode) {}

  Value getValueOrPlaceholder();
  Value getDriveConditionOrPlaceholder();
};
} // namespace
...
Value Def::getDriveConditionOrPlaceholder() {
  if (!mode.getCondition()) {
    auto builder = OpBuilder::atBlockBegin(block);
    Value condition;
    if (mode.isNever()) {
      condition = builder.create<hw::ConstantOp>(builder.getUnknownLoc(),
                                                 builder.getI1Type(), 0);
    } else if (mode.isAlways()) {
      condition = builder.create<hw::ConstantOp>(builder.getUnknownLoc(),
                                                 builder.getI1Type(), 1);
    } else {
      condition =
          builder
              .create<UnrealizedConversionCastOp>(
                  builder.getUnknownLoc(), builder.getI1Type(), ValueRange{})
              .getResult(0);
      driveConditionIsPlaceholder = true;
    }
    mode.setCondition(condition);
  }
  return mode.getCondition();
}

// Allow `DefDrive` to be used as hash map key.
template <>
struct llvm::DenseMapInfo<DefDrive> {
  static DefDrive getEmptyKey() {
    return DefDrive::Pair::getFromOpaqueValue(
        DenseMapInfo<void *>::getEmptyKey());
  }
  static DefDrive getTombstoneKey() {
    return DefDrive::Pair::getFromOpaqueValue(
        DenseMapInfo<void *>::getTombstoneKey());
  }
  static unsigned getHashValue(DefDrive d) {
    return DenseMapInfo<DefDrive::Pair>::getHashValue(d.conditionAndMode);
  }
  static bool isEqual(DefDrive lhs, DefDrive rhs) { return lhs == rhs; }
};
...
struct Lattice {
  ...
  Def *createDef(Value value, DefDrive mode) {
  ...
  /// comparisons.
  DenseMap<std::pair<Value, DefDrive>, Def *> defsForValues;
    ...
    for (auto &op : block.without_terminator()) {
      ...
      if (auto driveOp = dyn_cast<DrvOp>(op)) {
        ...
        if (!slotOrder.contains(driveOp.getSignal()))
          continue;
        auto *def = lattice.createDef(driveOp.getValue(), DefDrive::always());
...
/// propagate definitions from their predecessors into the block even if the
/// definitions are not available in all predecessors. This is overly
/// optimistic, but initially helps definitions propagate through loop
/// structures. If `optimisticMerges` is false, block entry points only
/// propagate definitions that are available in all predecessors.
...
void Promoter::propagateForward(LatticeNode *node, bool optimisticMerges) {
  ...
  if (auto *entry = dyn_cast<BlockEntry>(node)) {
    ...
    for (auto pair : reachingDefs) {
      ...
      DefDrive reachingDefMode = reachingDef->mode;
      ...
      if (!llvm::all_of(entry->predecessors, [&](auto *predecessor) {
            ...
            if (reachingDefMode != otherDef->mode)
              reachingDefMode = DefDrive::conditional();
            return true;
          }))
        continue;
...
void Promoter::insertProbes(BlockEntry *node) {
  ...
  for (auto neededDef : slots) {
    ...
    auto *def = lattice.createDef(value, DefDrive::never());
...
void Promoter::insertDrives(BlockExit *node) {
  ...
  for (auto slot : slots) {
    ...
    if (!reachingDef || reachingDef->mode.isNever())
      continue;
    ...
    auto enable = reachingDef->mode.isConditional()
                      ? reachingDef->getDriveConditionOrPlaceholder()
                      : Value{};
    builder.create<DrvOp>(slot.getLoc(), slot, value, time, enable);
  }
}
...
void Promoter::resolveDefinitions(ProbeNode *node) {
  if (!slotOrder.contains(node->slot))
    return;
  auto *def = node->valueBefore->reachingDefs.lookup(node->slot);
...
bool Promoter::insertBlockArgs(BlockEntry *node) {
  ...
  SmallVector<std::pair<Value, Which>> neededSlots;
  for (auto slot : slots) {
    if (auto *def = node->mergedDefs.lookup(slot)) {
      if (node->valueAfter->reachingDefs.contains(slot)) {
        ...
        if (def->driveConditionIsPlaceholder)
  ...
  for (auto [slot, which] : neededSlots) {
    ...
    case Which::Condition: {
      ...
      // earlier.
      auto *placeholder = def->mode.getCondition().getDefiningOp();
      assert(isa_and_nonnull<UnrealizedConversionCastOp>(placeholder) &&
             "placeholder replaced but driveConditionIsPlaceholder still set");
      auto conditionArg = node->block->addArgument(
          IntegerType::get(slot.getContext(), 1), slot.getLoc());
      replaceValueWith(placeholder->getResult(0), conditionArg);
      placeholder->erase();
      def->mode.setCondition(conditionArg);
      def->driveConditionIsPlaceholder = false;
      break;
    }
    }
  }
  ...
  for (auto *predecessor : node->predecessors) {
    ...
    for (auto [slot, which] : neededSlots) {
      ...
      assert(def && "no definition reaches terminator");
      switch (which) {
      case Which::Value:
        args.push_back(def->getValueOrPlaceholder());
        break;
      case Which::Condition:
        args.push_back(def->getDriveConditionOrPlaceholder());
        break;
      }
    }
...
void Promoter::replaceValueWith(Value oldValue, Value newValue) {
  ...
  for (auto *def : lattice.defs) {
    if (def->value == oldValue)
      def->value = newValue;
    if (def->mode.isConditional() && def->mode.getCondition() == oldValue)
      def->mode.setCondition(newValue);
  }
}
