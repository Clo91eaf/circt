...
hw.module @Precedence(in %clk: i1, in %a: i1, in %b: i1) {
  // CHECK: assert property (@(posedge clk) (a or ##0 b) and b);
  %a0 = ltl.clocked_delay %b, 0, 0 clock %clk, posedge : i1
  ...
  %d0 = ltl.clocked_delay %a1, 1, 0 clock %clk, posedge : !ltl.sequence
  sv.assert_property %d0 : !ltl.sequence

  // CHECK: assert property (@(posedge clk) not (a or ##0 b));
...
hw.module @SystemVerilogSpecExamples(in %clk: i1, in %a: i1, in %b: i1, in %c: i1, in %d: i1, in %e: i1) {
  ...
  // CHECK: assert property (@(posedge clk) a ##1 b ##0 c ##1 d);
  %a0 = ltl.clocked_delay %b, 1, 0 clock %clk, posedge : i1
  %a1 = ltl.clocked_delay %d, 1, 0 clock %clk, posedge : i1
  ...
  %b0 = ltl.clocked_delay %c, 1, 0 clock %clk, posedge : i1
  ...
  %d0 = ltl.clocked_delay %a, 1, 0 clock %clk, posedge : i1
...
hw.module @LivenessExample(in %clock: i1, in %reset: i1, in %isLive: i1) {
  ...
  %4 = ltl.clocked_delay %not_isLive, 1, 0 clock %clock, posedge : i1
