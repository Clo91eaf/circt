...
```
ltl.clocked_delay %input, <delay>[, <length>] clock %clock, <edge>
```

`%clock` is an `i1` value (e.g. a module clock); `<edge>` is `posedge`, `negedge`, or `edge`. For example, `ltl.clocked_delay %s, 3 clock %clk, posedge` means `%s` must hold 3 cycles later on `%clk` rising edges.
...
ltl.delay %s, 3
ltl.delay %s, 3, 0
ltl.clocked_delay %s, 3, 0 clock %clk, posedge
```
